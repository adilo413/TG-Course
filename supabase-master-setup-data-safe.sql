-- =====================================================
-- MASTER SUPABASE DATABASE SETUP (DATA-SAFE VERSION)
...
-- =====================================================
-- 1. CLEANUP EXISTING OBJECTS (SAFE - Only for tables that definitely exist)
-- =====================================================

-- Drop existing policies first (only for tables that might exist)
...
-- Drop existing triggers (only for tables that might exist)
...
-- 2.4 Courses Table (UPDATED - with proper column names)
...
-- =====================================================
-- 3. HANDLE EXISTING COURSES TABLE (If it exists with old structure)
-- =====================================================

-- Check if courses table has old 'subject' column and update it
DO $$ 
BEGIN
    -- If the old 'subject' column exists, rename it to 'subject_id'
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'courses' AND column_name = 'subject') THEN
        ALTER TABLE courses RENAME COLUMN subject TO subject_id;
    END IF;
    
    -- If the old 'chapter' column exists, rename it to 'chapter_id'
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'courses' AND column_name = 'chapter') THEN
        ALTER TABLE courses RENAME COLUMN chapter TO chapter_id;
    END IF;
END $$;

-- =====================================================
-- 4. INSERT DEFAULT DATA FIRST (Before foreign key constraints)
-- =====================================================
...
VALUES 
    ...
    -- Add common subject names that might exist in existing data
    ('ethiopia', 'Ethiopia', 'fas fa-globe', '#8e44ad', true),
    ('history', 'History', 'fas fa-landmark', '#e67e22', true),
    ('geography', 'Geography', 'fas fa-map', '#27ae60', true),
    ('civics', 'Civics', 'fas fa-balance-scale', '#34495e', true),
    ('science', 'Science', 'fas fa-flask', '#2ecc71', true),
    ('social', 'Social Studies', 'fas fa-users', '#9b59b6', true)
ON CONFLICT (subject_id) DO NOTHING;

-- 4.3 Insert Default Chapters for Each Subject
-- Amharic Chapters
...
-- Ethiopia Chapters (for existing data)
INSERT INTO chapters (chapter_id, subject_id, name, number, is_active)
VALUES 
    ('ethiopia_ch1', 'ethiopia', 'Chapter 1', 1, true),
    ('ethiopia_ch2', 'ethiopia', 'Chapter 2', 2, true),
    ('ethiopia_ch3', 'ethiopia', 'Chapter 3', 3, true),
    ('ethiopia_ch4', 'ethiopia', 'Chapter 4', 4, true),
    ('ethiopia_ch5', 'ethiopia', 'Chapter 5', 5, true),
    ('ethiopia_ch6', 'ethiopia', 'Chapter 6', 6, true)
ON CONFLICT (chapter_id) DO NOTHING;

-- 4.4 Insert Default Settings
...
-- =====================================================
-- 5. ADD FOREIGN KEY CONSTRAINTS (After all data is inserted)
-- =====================================================
...
ALTER TABLE courses 
ADD CONSTRAINT fk_courses_subject_id 
FOREIGN KEY (subject_id) REFERENCES subjects(subject_id) ON DELETE CASCADE;

ALTER TABLE courses 
ADD CONSTRAINT fk_courses_chapter_id 
FOREIGN KEY (chapter_id) REFERENCES chapters(chapter_id) ON DELETE SET NULL;

ALTER TABLE tokens 
ADD CONSTRAINT fk_tokens_course_id 
...
-- 6.1 Admin Password Verification Function
...
-- 6.2 Generate Unique Course ID Function
...
-- 6.3 Generate Unique Subject ID Function
...
-- 6.4 Generate Unique Chapter ID Function
...
-- 6.5 Update Updated At Column Function
...
-- 7.1 Admin Users Triggers
CREATE TRIGGER update_admin_users_updated_at 
    BEFORE UPDATE ON admin_users
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 7.2 Subjects Triggers
CREATE TRIGGER update_subjects_updated_at 
    BEFORE UPDATE ON subjects
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 7.3 Chapters Triggers
CREATE TRIGGER update_chapters_updated_at 
    BEFORE UPDATE ON chapters
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 7.4 Courses Triggers
CREATE TRIGGER update_courses_updated_at 
    BEFORE UPDATE ON courses
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- 7.5 Settings Triggers
...
-- Show existing courses data
SELECT 'Existing courses:' as info, 
       'Total courses: ' || (SELECT COUNT(*) FROM courses) as total_courses,
       'Unique subjects in courses: ' || (SELECT COUNT(DISTINCT subject_id) FROM courses) as unique_subjects;
